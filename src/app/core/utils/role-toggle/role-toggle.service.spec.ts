...
describe('Role Toggle Service', () => {
  ...
  describe('hasRole', () => {
    ...
    it('should return shortcut true for role "always"', () => {
      expect(roleToggleService.hasRole('always')).toBeObservable(cold('(a|)', { a: true }));
    });

    it('should return shortcut false for role "never"', () => {
      expect(roleToggleService.hasRole('never')).toBeObservable(cold('(a|)', { a: false }));
    });
  });
});
